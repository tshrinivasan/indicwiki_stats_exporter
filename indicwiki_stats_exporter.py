...
wiki_info = yaml.load(open(os.path.join(THIS_DIR, "indic_wiki.yaml")))
indic_wikipedia = wiki_info["indic_wikipedia"].split(",")

with open(os.path.join(THIS_DIR, "languages.json")) as f:
    ...
    def collect(self):
        try:
            wiki_pages = GaugeMetricFamily(
                "wiki_pages", "wiki_pages", labels=["lang_code", "language"]
            )
            ...
            wiki_edits = GaugeMetricFamily(
                "wiki_edits", "wiki_edits", labels=["lang_code", "language"]
            )

            wiki_images = GaugeMetricFamily(
                "wiki_images", "wiki_images", labels=["lang_code", "language"]
            )

            wiki_users = GaugeMetricFamily(
                "wiki_users", "wiki_users", labels=["lang_code", "language"]
            )
            ...
            wiki_admins = GaugeMetricFamily(
                "wiki_admins", "wiki_admins", labels=["lang_code", "language"]
            )
